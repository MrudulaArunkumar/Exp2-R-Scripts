...
Exp2_HiwiPilot <- Exp2_HiwiPilot%>%group_by(participant)%>%fill(Screen_bg,.direction = "down")
...
#aggregate
Exp2agg_l_hp_fo <- aggregate(data = Exp2learn_HiwiPilot,RT_ifo~participant+Validity+Saliency,mean)
...
-   Outliers
    ...
    #aggregate
    Exp2agg_l_hp_o <- aggregate(data = Exp2learn_HiwiPilot,RT_io~participant+Validity+Saliency,mean)
...
-   Farouts
  ...
  in the pilot data from Hiwis, there is still no significant effect but the validity is almost significant with p = .10
  Also in the right direction as per the graph
...
-   Farouts
  When position of the number is factored in the validity is almost significant (p = .09), also in the direction where top position is faster
...
-   Outliers
      ...
      There seems to be a trend heading towards a significant interaction between validity and position of number. 
