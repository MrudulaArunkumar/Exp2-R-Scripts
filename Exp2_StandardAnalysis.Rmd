...
CA_TotalAcc <- Exp2_CA %>%
 ...
 CA_TotalAcc$MeanAcc <- Exp2_CA %>%
  dplyr::group_by(participant) %>%
  dplyr::summarise(MeanAcc= mean(CA_RespKey.corr, na.rm=TRUE))

summary(CA_TotalAcc$TotalAcc)


##funnelled Questionnaire
QuesCA <- Exp2_CA %>%
  dplyr::group_by(participant)%>%
  mutate(EvenYes = ifelse(Exp2_CA$Questionnaire.thisN == 0, sum(Exp2_CA$AwarenessResp.keys == "j"), NA))
