---
title: "Exp2_pilotdata"
author: "Mrudula"
date: "22/10/2020"
output: html_document
---

This is the Pilot Data Analysis collected from Prolific and Hiwis for the second version of the Experiment that brought back the distractor words towards the center and the number appears either at the top or bottom.
...
#loading pilot
Hiwis <- read.csv("HiwiPilot.csv")
Prolific <- read.csv("ProlificPilot.csv")
...
Exp2_Pilot <- rbind(Hiwis, Prolific)


#write.csv(Exp2_Pilot, file = "Pilot.csv")
attach(Exp2_Pilot)
...
Exp2_Pilot <- Exp2_Pilot%>%group_by(participant)%>%fill(Screen_bg,.direction = "down")

Exp2_Pilot <- Exp2_Pilot %>% group_by(participant)%>%fill(blocks.thisN,.direction = "up")

Exp2_Pilot <- rename(Exp2_Pilot, c("blocks.thisN" = "BlockCount"))

Exp2_CA <- Exp2_Pilot %>%
  filter(Condition == "ContChkTest" | str_detect(AwareQ, "mit"))

#adjusting RT
Exp2_Pilot <- separate(Exp2_Pilot, col = ResponseKey.rt, into = c("RT_Trials", "RT_secondary"), sep = ',')
Exp2_Pilot$RT_Trials <- Exp2_Pilot$RT_Trials%>%
  str_replace_all("\\[|\\]","")%>%
  as.double(Exp2_Pilot$RT_Trials)
Exp2_Pilot$RT_Trials <- 1000*(Exp2_Pilot$RT_Trials)
Exp2_Pilot$PreTargetDisplayTime <- 1000*(Exp2_Pilot$PreTargetDisplayTime)

Exp2_Pilot <- Exp2_Pilot%>%drop_na(RT_Trials)

Exp2_Pilot$ACC_trials <- Exp2_Pilot$ResponseKey.corr
Exp2_Pilot$ErrorRate <- 1 - Exp2_Pilot$ACC_trials
...
```{r demo, echo=FALSE}
Exp2_Pilot$Age[Exp2_Pilot$Age == '"$'] <- 24
Exp2_Pilot$Age <- as.numeric(Exp2_Pilot$Age)
pander(summary(Exp2_Pilot$Age), style = 'rmarkdown', caption = "Mean Age of participants")

kable((table(Exp2_Pilot$Gender))/480, format = "html", caption = " Gender distribution")
```

Summary of the overall RT

```{r descriptive, echo=FALSE}
pander(summary(Exp2_Pilot$RT_Trials), style = 'rmarkdown',caption = 'Mean RT')
pander(table(Exp2_Pilot$ACC_trials),style = 'rmarkdown',caption = "Accuracy")
...
Exp2_Pilot$RT_Trials[Exp2_Pilot$ACC_trials==0] <- NA
...
Exp2_Pilot <- ddply(Exp2_Pilot, .(participant), computeTukeys)

#creating new column with RT trials after removing outliers/farouts
Exp2_Pilot$RT_ifo <- Exp2_Pilot$RT_Trials
Exp2_Pilot$RT_io <- Exp2_Pilot$RT_Trials
Exp2_Pilot$RT_ifo[Exp2_Pilot$RT_ifo > Exp2_Pilot$Farouts|Exp2_Pilot$RT_ifo < 300] <- NA
Exp2_Pilot$RT_io[Exp2_Pilot$RT_io > Exp2_Pilot$Outlier|Exp2_Pilot$RT_io < 300] <- NA

pander(summary(Exp2_Pilot$RT_ifo), style = 'rmarkdown', caption = "Summary of RT after removing Farouts")
pander(summary(Exp2_Pilot$RT_io), style = 'rmarkdown', caption = "Summary of RT after removing Outliers")
...
Exp2agg <- aggregate(data = Exp2_Pilot,RT_ifo~participant+Validity+Condition,mean)
...
```{r}
Exp2learn_Pilot <- Exp2_Pilot %>%
  filter(Condition == "learn")

Exp2test_Pilot <- Exp2_Pilot %>%
  filter(Condition == "test")

Exp2learn_Pilot<- Exp2learn_Pilot%>%
...
#aggregate
Exp2agg_l_pp_fo <- aggregate(data = Exp2learn_Pilot,RT_ifo~participant+Validity,mean)
# kable(Exp2agg_l_pp_fo, format = "html", caption = "Summary of RTs of valid and invalid trials")

#convert to wide
Exp2agg_l_pp_fo_wide <- spread(Exp2agg_l_pp_fo, Validity,RT_ifo)
Exp2agg_l_pp_fo_wide$ValEffect <- Exp2agg_l_pp_fo_wide$invalid - Exp2agg_l_pp_fo_wide$valid
Exp2agg_l_pp_fo_wide$participant <- c(1,2,3,4,5,6,7,8)
pander(t.test(RT_ifo~Validity, data = Exp2agg_l_pp_fo, paired = TRUE), style = 'rmarkdown')
...
```{r echo = FALSE}
ggplot(data = Exp2agg_l_pp_fo_wide, aes(x = participant, y = ValEffect))+
...
T test revealed a p value of .09 for learn valid and invalid trials again in the same direction as the farouts where invalid trials are faster than valid
    ...
    #aggregate
Exp2agg_l_pp_o <- aggregate(data = Exp2learn_Pilot,RT_io~participant+Validity,mean)
Exp2agg_l_pp_o_wide <- spread(Exp2agg_l_pp_o, Validity,RT_io)
Exp2agg_l_pp_o_wide$ValEffect <- Exp2agg_l_pp_o_wide$invalid - Exp2agg_l_pp_o_wide$valid
Exp2agg_l_pp_o_wide$participant <- c(1,2,3,4,5,6,7,8)

pander(t.test(RT_io~Validity, data = Exp2agg_l_pp_o, paired = TRUE), style = 'rmarkdown')

ggplot(data = Exp2agg_l_pp_o_wide, aes(x = participant, y = ValEffect))+
  geom_bar(stat = "identity", fill = "darkslateblue")+
  theme_classic()+
...
Exp2agg_l_pp_ER <- aggregate(data = Exp2learn_Pilot,ErrorRate~participant+Validity,mean)

pander(t.test(ErrorRate~Validity, data =Exp2agg_l_pp_ER, paired = TRUE), style = 'rmarkdown', caption = "t test results for error Rates", split.table = "Inf", missing = NA)

ggplot(data = Exp2agg_l_pp_ER, aes(x = Validity, y = ErrorRate, fill = Validity))+
...
Exp2test_Pilot <- Exp2test_Pilot %>%
  select(Validity,Saliency,Condition,PositionD,PositionT,Distractor1,Distractor2,everything())

#aggregate
Exp2agg_t_pp_fo <- aggregate(data = Exp2test_Pilot,RT_ifo~participant+Validity+Saliency,mean)
# 
...
```{r echo =  FALSE}
Exp2valeffect_tfo <- spread(Exp2agg_t_pp_fo, Validity, RT_ifo)
...
Exp2agg_t_pp_o <- aggregate(data = Exp2test_Pilot,RT_io~participant+Validity+Saliency,mean)
...
```{r echo = FALSE}
Exp2valeffect_to <- spread(Exp2agg_t_pp_o, Validity, RT_io)
...
Exp2agg_t_pp_ER <- aggregate(data = Exp2test_Pilot,ErrorRate~participant+Validity+Saliency,mean)
...
ggplot(data = Exp2agg_t_pp_ER, aes(x = Validity, y = ErrorRate, fill = Validity))+
  geom_col()+
...
#aggregate
Exp2agg_lp_pp_fo <- aggregate(data = Exp2learn_Pilot,RT_ifo~participant+Validity+PositionMatch,mean)
...
#aggregate
Exp2agg_tp_pp_fo <- aggregate(data = Exp2test_Pilot,RT_ifo~participant+Validity+Saliency+PositionT,mean)
...
Exp2agg_tp_pp_o <- aggregate(data = Exp2test_Pilot,RT_io~participant+Validity+Saliency+PositionT,mean)
...
Exp2agg_tp_pp_ER <- aggregate(data = Exp2test_Pilot,ErrorRate~participant+Validity+PositionT+Saliency,mean)
